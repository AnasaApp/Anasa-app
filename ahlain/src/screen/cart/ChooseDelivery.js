...
const ChooseDelivery = ({navigation, route}) => {
  ...
  useEffect(() => {
    if (BookingEligiblityResponse != null) {
      if (BookingEligiblityResponse?.error == false) {
        if (
          Number(myCartData?.grandTotal) +
            Number(totalDeliveryCharges) -
            Number(route?.params?.usedWalletBalance) >
          0
        ) {
          onMakePaymentApi();
        } else {
  ...
  const handleStartTimeConfirm = time => {
    ...
    const currentDateDiff = moment().calendar(schedule_start_date || new Date(), {
      sameDay: '[Today]',
      nextDay: '[Tomorrow]',
      nextWeek: 'dddd',
      lastDay: '[Yesterday]',
  ...
  const handleEndTimeConfirm = time => {
    const currentTimeDiff = moment().diff(time, 'minutes');
    // Parse schedule_start_time with format since it's stored as 'hh:mm A' string
    const startTimeMoment = schedule_start_time
      ? moment(schedule_start_time, 'hh:mm A')
      : null;
    const endTimeDiff = startTimeMoment
      ? startTimeMoment.diff(moment(time), 'minutes')
      : -1;
    const currentDateDiff = moment().calendar(schedule_end_date || new Date(), {
      sameDay: '[Today]',
      nextDay: '[Tomorrow]',
      nextWeek: 'dddd',
      lastDay: '[Yesterday]',
      ...
      <ScrollView
        ...
        <RBSheet
          ...
          <DateTimePickerModal
            ...
            mode="date"
            date={schedule_start_date ? new Date(schedule_start_date) : new Date()}
          ...
          <DateTimePickerModal
            ...
            mode="date"
            date={schedule_end_date ? new Date(schedule_end_date) : new Date()}
